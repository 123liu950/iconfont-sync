...
function parseArgs() {
  ...
  if (Object.values(envArgs).length > 0) {
    return envArgs;
  }
...
async function login(config) {
  ...
  try {
    ...
    const resp = await axios.post('https://www.iconfont.cn/api/account/login.json', {
...
async function downloadIcons(config, tempDir) {
  ...
  try {
    const resp = await axios.get('https://www.iconfont.cn/api/project/download.zip', {
...
function compareAndUpdate(config, tempDir) {
  ...
  console.log('   📝 注意: 文本文件会忽略时间戳差异，字体文件根据 CSS 变化判断');
  console.log('');
  ...
  for (const fileName of config.files) {
    ...
    if (isBinaryFile(fileName)) {
      const shouldUpdate = shouldUpdateBinaryBasedOnCSS(fileName, config, tempDir);
